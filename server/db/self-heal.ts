...
const STEPS: HealingStep[] = [
  ...
  {
    name: 'saved results: fundamental scanner + portfolio optimizer',
    run: async () => {
      let createdSomething = false;
      ...
      if (!(await tableExists('saved_fundamental_screener_results'))) {
        await query(`
          CREATE TABLE saved_fundamental_screener_results (
        ...
        `);
        await query(`CREATE INDEX IF NOT EXISTS idx_saved_fundamental_screener_user ON saved_fundamental_screener_results(user_id, created_at DESC)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_saved_fundamental_screener_shared ON saved_fundamental_screener_results(share_token) WHERE share_token IS NOT NULL`);
        console.log('[SELF_HEAL]   • Created saved_fundamental_screener_results');
        createdSomething = true;
      }

      if (!(await tableExists('saved_portfolio_optimizer_results'))) {
        await query(`
          CREATE TABLE saved_portfolio_optimizer_results (
            id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
            user_id UUID REFERENCES users(id) ON DELETE SET NULL,
            name VARCHAR(255) NOT NULL,
        ...
        `);
        await query(`CREATE INDEX IF NOT EXISTS idx_saved_portfolio_optimizer_user ON saved_portfolio_optimizer_results(user_id, created_at DESC)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_saved_portfolio_optimizer_shared ON saved_portfolio_optimizer_results(share_token) WHERE share_token IS NOT NULL`);
        console.log('[SELF_HEAL]   • Created saved_portfolio_optimizer_results');
        createdSomething = true;
      }

      const defaults: Array<[string, string, string]> = [
        ['saved_fundamental_screener_limit_basic', '10', 'Max saved fundamental scanner results for basic tier'],
        ['saved_fundamental_screener_limit_premium', '50', 'Max saved fundamental scanner results for semi/pro tier'],
        ['saved_portfolio_optimizer_limit_basic', '10', 'Max saved portfolio optimizer results for basic tier'],
        ['saved_portfolio_optimizer_limit_premium', '50', 'Max saved portfolio optimizer results for semi/pro tier'],
      ];
      ...
      for (const [key, value, description] of defaults) {
        ...
        if (r.rowCount && r.rowCount > 0) {
          console.log(`[SELF_HEAL]   • Seeded system_config[${key}] = ${value}`);
          createdSomething = true;
        }
      }
